...
export const WrongPredictionsMusic = pgTable("wrong_predictions_music", {
  id: serial("id").primaryKey(),
  walletAddress: text("walletAddress").notNull(),
  wrongPredictionDate: text("wrong_prediction_date").notNull(), // Date they made wrong prediction
  createdAt: timestamp("created_at").defaultNow().notNull(),
...
export const WrongPredictionsFormula1 = pgTable("wrong_predictions_formula1", {
  id: serial("id").primaryKey(),
  walletAddress: text("walletAddress").notNull(),
  wrongPredictionDate: text("wrong_prediction_date").notNull(), // Date they made wrong prediction
  createdAt: timestamp("created_at").defaultNow().notNull(),
...
export const MusicBets = pgTable("music_bets", {
  id: serial("id").primaryKey(), // Auto-incrementing ID as primary key
  walletAddress: text("wallet_address").notNull(), // Bettor's wallet address
  prediction: text("prediction").notNull(), // "positive" or "negative"
  betDate: text("bet_date").notNull(), // Date of the bet (YYYY-MM-DD format)
...
export const Formula1Bets = pgTable("formula1_bets", {
  id: serial("id").primaryKey(), // Auto-incrementing ID as primary key
  walletAddress: text("wallet_address").notNull(), // Bettor's wallet address
  prediction: text("prediction").notNull(), // "positive" or "negative"
  betDate: text("bet_date").notNull(), // Date of the bet (YYYY-MM-DD format)
...
// Pot information tracking - stores metadata for each prediction pot
export const PotInformation = pgTable("pot_information", {
  id: serial("id").primaryKey(),
  contractAddress: text("contract_address").notNull().unique(), // Contract address (unique identifier)
  hasStarted: boolean("has_started").default(false).notNull(), // Whether the pot has started
  ...
  emailsSent: boolean("emails_sent").default(false).notNull(), // Whether minimum players emails were sent
